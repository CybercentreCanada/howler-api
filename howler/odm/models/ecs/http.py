...
@odm.model(
    index=True, store=True, description="Defines the body of a request/response."
)
...
@odm.model(
    index=True, store=True, description="These fields can represent errors of any kind."
)
class Request(odm.Model):
    ...
    mime_type = odm.Optional(
        odm.Keyword(description="Mime type of the body of the request.")
    )
...
class Response(odm.Model):
    body = odm.Optional(
        odm.Compound(Body, description="Defines the body of a response")
    )
    bytes = odm.Optional(
        odm.Integer(
            description="Total size in bytes of the request (body and headers)."
