...
class File(odm.Model):
    ...
    attributes = odm.Optional(odm.List(odm.Keyword(), description="Array of file attributes."))
    created = odm.Optional(odm.Date(description="File creation time."))
    ctime = odm.Optional(odm.Date(description="Last time the file attributes or metadata changed."))
    device = odm.Optional(odm.Keyword(description="Device that is the source of the file."))
    ...
    extension = odm.Optional(odm.Keyword(description="File extension, excluding the leading dot."))
    fork_name = odm.Optional(odm.Keyword(description="A fork is additional data associated with a filesystem object."))
    ...
    inode = odm.Optional(odm.Keyword(description="Inode representing the file in the filesystem."))
    ...
    mode = odm.Optional(odm.Keyword(description="Mode of the file in octal representation."))
    mtime = odm.Optional(odm.Date(description="Last time the file content was modified."))
    name = odm.Optional(odm.Keyword(description="Name of the file including the extension, without the directory."))
    ...
    type = odm.Optional(odm.Enum(values=FILE_TYPE, description="File type (file, dir, or symlink)."))
    uid = odm.Optional(odm.Keyword(description="The user ID (UID) or security identifier (SID) of the file owner."))
