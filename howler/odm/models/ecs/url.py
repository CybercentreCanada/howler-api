...
class URL(odm.Model):
    domain = odm.Optional(
        odm.Domain(description='Domain of the url, such as "www.elastic.co".')
    )
    ...
    fragment = odm.Optional(
        odm.Keyword(description='Portion of the url after the #, such as "top".')
    )
    full = odm.Optional(
        odm.Keyword(
    ...
    original = odm.Optional(
        odm.Keyword(description="Unmodified original url as seen in the event source.")
    )
    password = odm.Optional(odm.Keyword(description="Password of the request."))
    path = odm.Optional(
        odm.Keyword(description='Path of the request, such as "/search".')
    )
    ...
    registered_domain = odm.Optional(
        odm.Keyword(
            description="The highest registered url domain, "
            "stripped of the subdomain."
        )
    )
    scheme = odm.Optional(
        odm.Keyword(description='Scheme of the request, such as "https".')
    )
