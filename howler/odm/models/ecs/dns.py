...
@odm.model(
    index=True, store=True, description="An answer section returned by the server."
)
class DNSAnswer(odm.Model):
    ...
    type = odm.Optional(
        odm.Keyword(description="The type of data contained in this resource record.")
    )
...
class DNSQuestion(odm.Model):
    class_ = odm.Optional(
        odm.Keyword(description="The class of records being queried.")
    )
...
@odm.model(
    index=True, store=True, description="Fields describing DNS queries and answers."
)
class DNS(odm.Model):
    ...
    header_flags = odm.Optional(
        odm.List(odm.Keyword(), description="Array of 2 letter DNS header flags.")
    )
    id = odm.Optional(
        odm.Keyword(
            description="The DNS packet identifier assigned by the program that generated the query."
        )
    )
    ...
    resolved_ip = odm.Optional(
        odm.List(odm.IP(), description="Array containing all IPs seen in answers.data.")
    )
    response_code = odm.Optional(odm.Keyword(description="The DNS response code."))
    type = odm.Optional(
        odm.Keyword(description="The type of DNS event captured, query or answer.")
    )
