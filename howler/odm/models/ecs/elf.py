...
class Section(odm.Model):
    chi2 = odm.Optional(
        odm.Integer(description="Chi-square probability distribution of the section.")
    )
    entropy = odm.Optional(
        odm.Integer(description="Shannon entropy calculation from the section.")
    )
    ...
    physical_size = odm.Optional(
        odm.Integer(description="ELF Section List physical size.")
    )
    type = odm.Optional(odm.Keyword(description="ELF Section List type."))
    virtual_address = odm.Optional(
        odm.Integer(description="ELF Section List virtual address.")
    )
    virtual_size = odm.Optional(
        odm.Integer(description="ELF Section List virtual size.")
    )


@odm.model(index=True, store=True, description="Header information about the ELF file.")
class Header(odm.Model):
    abi_version = odm.Optional(
        odm.Keyword(
    ...
    entrypoint = odm.Optional(
        odm.Integer(description="Header entrypoint of the ELF file.")
    )
    object_version = odm.Optional(
        odm.Keyword(description="'0x1' for original ELF files.")
    )
    os_abi = odm.Optional(
        odm.Keyword(description="Application Binary Interface (ABI) of the Linux OS.")
    )
...
class ELF(odm.Model):
    architecture = odm.Optional(
        odm.Keyword(description="Machine architecture of the ELF file.")
    )
    ...
    creation_date = odm.Optional(
        odm.Keyword(description="Extracted when possible from the file’s metadata.")
    )
    exports = odm.List(
        odm.Keyword(),
        default=[],
    ...
    header = odm.Optional(
        odm.Compound(Header, description="Header information about the ELF file.")
    )
    imports = odm.Optional(
        odm.List(odm.Keyword(), description="List of imported element names and types.")
    )
    ...
    telfhash = odm.Optional(
        odm.Keyword(description="telfhash symbol hash for ELF file.")
    )
