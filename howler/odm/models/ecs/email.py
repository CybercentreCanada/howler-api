...
class File(odm.Model):
    extension = odm.Optional(
        odm.Keyword(description="Attachment file extension, excluding the leading dot.")
    )
    hash = odm.Optional(
        odm.Compound(Hashes, description="Hashes, usually file hashes.")
    )
    mime_type = odm.Optional(
        odm.Keyword(description="The MIME media type of the attachment.")
    )
    name = odm.Optional(
        odm.Keyword(
...
class Attachment(odm.Model):
    file = odm.Optional(
        odm.Compound(File, description="Information about the file sent.")
    )
...
class ParentEmail(odm.Model):
    bcc = odm.Optional(
        odm.Compound(Address, description="The email address of BCC recipient.")
    )
    cc = odm.Optional(
        odm.Compound(Address, description="The email address of CC recipient.")
    )
    from_ = odm.Optional(
        odm.Compound(
            Address,
            description="The email address of the sender, typically "
            "from the RFC 5322 From: header field.",
        )
    )
    ...
    origination_timestamp = odm.Optional(
        odm.Date(description="The date and time the email message was composed.")
    )
    subject = odm.Optional(
        odm.Keyword(description="A brief summary of the topic of the message.")
    )
    to = odm.Optional(
        odm.Compound(Address, description="The email address of recipient.")
    )
...
class Email(odm.Model):
    ...
    content_type = odm.Optional(
        odm.Keyword(description="Information about how the message is to be displayed.")
    )
    delivery_timestamp = odm.Optional(
        odm.Date(
            description="The date and time when the email message "
            "was received by the service or client."
        )
    )
    direction = odm.Optional(
        odm.Keyword(
            description="The direction of the message based on the "
    ...
    local_id = odm.Optional(
        odm.Keyword(
            description="Unique identifier given to the email by the source "
