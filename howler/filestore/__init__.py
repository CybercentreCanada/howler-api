...
from howler.filestore.transport.sftp import TransportSFTP
from howler.odm.models.config import Host

log = get_logger(__file__)

if TYPE_CHECKING:
    from howler.filestore.transport.base import Transport
        ...
        for t in self.slice(location):
            if force or not t.exists(dst_path):
                ...
                if not t.exists(dst_path):
                    raise FileStoreException(
                        "File transfer failed. Remote file does not "
